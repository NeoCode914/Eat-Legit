...
import { partnerSignupSchema, type PartnerSignupSchema } from "../schemas/auth";
import { useAuth } from "../context/AuthContext";
import { Eye, EyeOff, ChefHat, ArrowRight, Building2, Mail, Lock } from "lucide-react";
...
export default function PartnerSignup() {
  ...
  } = useForm<PartnerSignupSchema>({
    resolver: zodResolver(partnerSignupSchema),
  });

  const onSubmit = useCallback(
    async (values: PartnerSignupSchema) => {
      setServerError("");
      try {
        await signUpPartner({
          name: values.name,
        ...
        <div className="relative space-y-6">
          <div className="space-y-3">
            ...
            <p className="text-white/50 text-lg leading-relaxed">
              Showcase your dishes to thousands of hungry food lovers in your area.
            </p>
          </div>

          <div className="grid grid-cols-2 gap-3">
            {[
              { emoji: "🎬", label: "Upload Videos", desc: "Share your food stories" },
              { emoji: "📈", label: "Grow Reach", desc: "Gain new customers" },
              { emoji: "❤️", label: "Get Likes", desc: "Build your fanbase" },
              { emoji: "🏷️", label: "Free to Join", desc: "No hidden fees" },
            ].map((item) => (
              <div
                key={item.label}
      ...
      <div className="flex-1 flex items-center justify-center p-6 lg:p-12">
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="space-y-2">
            ...
            <h2 className="text-3xl font-black text-white">Partner signup</h2>
            <p className="text-white/40 text-sm">
              Register your restaurant or food brand
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
            ...
            <div className="space-y-1.5">
              <label htmlFor="partner-signup-name" className="text-white/70 text-sm font-medium">
            ...
            {/* Email */}
            <div className="space-y-1.5">
              <label htmlFor="partner-signup-email" className="text-white/70 text-sm font-medium">
                Business Email
              </label>
              <div className="relative">
                ...
                <input
                  ...
                  {...register("email")}
                  className="w-full pl-10 pr-4 py-3 rounded-2xl bg-white/5 border border-white/10 text-white placeholder:text-white/25 text-sm focus:outline-none focus:border-emerald-500/60 transition-all duration-200"
                />
              </div>
              {errors.email && (
            ...
            {/* Password */}
            <div className="space-y-1.5">
              <label htmlFor="partner-signup-password" className="text-white/70 text-sm font-medium">
                Password
              </label>
              <div className="relative">
            ...
            {/* Confirm Password */}
            <div className="space-y-1.5">
              <label htmlFor="partner-signup-confirm" className="text-white/70 text-sm font-medium">
                Confirm Password
              </label>
              <div className="relative">
                <Lock size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-white/30" />
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  Create Partner Account <ArrowRight size={15} />
                </>
              )}
            </button>
          </form>
          ...
          <div className="space-y-3 text-center text-sm">
            <p className="text-white/40">
              ...
              <Link to="/partner/signin" className="text-emerald-400 hover:text-emerald-300 font-medium transition-colors">
                Sign In
              </Link>
            </p>
